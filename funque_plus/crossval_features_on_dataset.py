...
def print_agg_stats(stats: List[Dict[str, Any]]) -> None:
    """
    stats: list of dicts, e.g. [
        {'SROCC': 0.91, 'PLCC': 0.90, 'KROCC': 0.88, 'RMSE': 3.2},
        ...
    ]
    """
    ...
    print('Stat,Median,LoCI,HiCI,Std')  # CSV-friendly
    for stat_key in stats[0]:
        values = np.array([s[stat_key] for s in stats])
        print(f'{stat_key},{np.median(values):.4f},'
              f'{np.percentile(values, lo_ci):.4f},'
              f'{np.percentile(values, hi_ci):.4f},'
              f'{np.std(values):.4f}')
...
def main() -> None:
    ...
    if args.out_file is not None:
      ...
      json_path = args.out_file.rsplit('.', 1)[0] + ".json"
      with open(json_path, 'w') as jf:
        json.dump(agg_stats, jf, indent=2)
