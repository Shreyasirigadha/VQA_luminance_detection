...
def main():
    ...
    if args.out_file is not None:
      ...
      # Convert safely for saving
      try:
        result_dict = {
            'agg_feats': result.agg_feats if result.agg_feats is not None else 0,
            'feat_names': result.feat_names if result.feat_names is not None else [],
            'score': getattr(result, 'score', 0)
        }
      except Exception as e:
        print(f"⚠️ Could not extract full result info: {e}")
        result_dict = {'error': str(e)}

      from scipy.io import savemat
      savemat(args.out_file, result_dict)
      print(f"✅ Results saved successfully to {args.out_file}")
